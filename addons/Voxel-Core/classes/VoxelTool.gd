...
func add_face(
	...
	match face:
		Vector3.RIGHT:
			if _uv_voxels:
				surface.surface_tool.add_uv((uv + Vector2.RIGHT) * _voxel_set.get_uv_scale())
			surface.surface_tool.add_vertex((top_left + Vector3.RIGHT + Vector3.UP) * Voxel.VoxelWorldSize)
			if _uv_voxels:
				surface.surface_tool.add_uv((uv + Vector2.ONE) * _voxel_set.get_uv_scale())
			surface.surface_tool.add_vertex((bottom_left + Vector3.RIGHT) * Voxel.VoxelWorldSize)
			if _uv_voxels:
				surface.surface_tool.add_uv((uv) * _voxel_set.get_uv_scale())
			surface.surface_tool.add_vertex((top_right + Vector3.ONE) * Voxel.VoxelWorldSize)
			if _uv_voxels:
				surface.surface_tool.add_uv((uv + Vector2.DOWN) * _voxel_set.get_uv_scale())
